...
SESSION_COOKIE = "sts_swing_session"
_SALT = "sts-swing-ranking-dashboard-session"
_MUTATING = {"POST", "PUT", "PATCH", "DELETE"}
_OPEN_PREFIXES = ("/auth/", "/assets/")
_OPEN_PATHS = {"/healthz", "/login", "/favicon.svg"}
USERS_FILE = Path("configs") / "dashboard_users.yaml"
...
def _redirect_uri(request: Request) -> str:
    """The callback URL registered on the Google credential.

    Through an IAP tunnel the request host is whatever the operator typed,
    so `url_for` alone yields `127.0.0.1` for one person and `localhost` for
    the next while Google demands an exact match. DASHBOARD_OAUTH_REDIRECT_URI
    pins it; the default is the tunnelled 8010 callback.
    """
    override = os.environ.get("DASHBOARD_OAUTH_REDIRECT_URI")
    return override or str(request.url_for("google_callback"))


def install_auth_routes(app, repo_root: Path, secret: str) -> None:
    from fastapi import Form

    users_path = Path(repo_root) / USERS_FILE
        ...
        if role is None:
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "detail": "This account is not on the access list.",
                },
            )
